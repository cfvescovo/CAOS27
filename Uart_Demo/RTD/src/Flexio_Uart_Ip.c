...
#define FLEXIO_UART_IP_SW_MAJOR_VERSION_C               3
                                       ...
                                       const Flexio_Uart_Ip_TimerDecrementType TimerDec)
{
    ...
    /* (Divider = InputClock / (BaudRate * 32U or 512U depend on baudrate) - 1; */
    Flexio_Uart_Ip_SetTimerCompare(Base, TIMER(Channel), (uint16)((((uint16)(Bits << 1U) - 1U) << 8U) + Divider));
                                                     ...
                                                     const uint32 ClockFrequency)
{
    ...
    else
    {
        ...
        Flexio_Uart_Ip_SetTimerCompare(Base, TIMER(Channel), (uint16)((((uint16)(BitCountValue << 1U) - 1U) << 8U) + (uint8)Divider));
